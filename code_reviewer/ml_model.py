...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
# CORRECTED IMPORT: Matches the function name in your feature_extractor.py
from .feature_extractor import extract_features_from_snippet
...
class RiskModel:
    def __init__(self):
        model_path = os.path.join('models', 'risk_assessment_model.pkl')
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Model file not found at {model_path}. Run scripts/train_model.py.")
    ...
    def predict(self, code_snippet):
        # 1. Extract the SAME 3 features using the correct function
        features = extract_features_from_snippet(code_snippet)
        
        # 2. Scale them using the saved scaler
        # Reshape to (1, 3) because the scaler expects a 2D array
        scaled_features = self.scaler.transform([features])
